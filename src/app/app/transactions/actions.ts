...
export async function setGroup(
  ...
  if (!(await ownsTransaction(db(), userId, txnId))) return;
...
export async function setComment(txnId: string, comment: string) {
  if (!(await ownsTransaction(db(), await requireUserId(), txnId))) return;
  await setTransactionComment(db(), txnId, normalizeComment(comment));
  revalidateAll();
}
...
export async function setIgnored(txnId: string, ignored: boolean) {
  if (!(await ownsTransaction(db(), await requireUserId(), txnId))) return;
  await setTransactionIgnored(db(), txnId, ignored);
  revalidateAll();
}
...
async function groupeTenable(form: ManualFormInput): Promise<number | null> {
...
export async function addTransaction(form: ManualFormInput) {
  ...
  const userId = await requireUserId();
  if (!(await ownsAccount(db(), userId, form.accountId))) return;
  if (form.groupId !== null && !(await ownsGroup(db(), userId, form.groupId))) return;
  await insertManualTransaction(db(), { ...toManualInput(form), groupId: await groupeTenable(form) });
  revalidateAll();
}

export async function editTransaction(id: string, form: ManualFormInput) {
  if (!(await ownsTransaction(db(), await requireUserId(), id))) return;
  if (!isValidManualForm(form)) return;
  const { accountId: _accountId, ...rest } = toManualInput(form);
  await updateManualTransaction(db(), id, { ...rest, groupId: await groupeTenable(form) });
  revalidateAll();
}
...
export async function mergeTransaction(syncedId: string, manualId: string) {
  const userId = await requireUserId();
  if (!(await ownsTransaction(db(), userId, syncedId)) || !(await ownsTransaction(db(), userId, manualId))) return;
  await mergeTransactions(db(), { syncedId, manualId });
  revalidateAll();
}
