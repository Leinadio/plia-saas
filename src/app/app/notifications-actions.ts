...
async function siennes(db: Db, userId: string, ids: string[]): Promise<string[]> {
  const vus = new Map<string, boolean>();
  const gardees: string[] = [];
  for (const id of ids) {
    const compte = id.split("::")[0];
    if (!compte) continue;
    if (!vus.has(compte)) vus.set(compte, await ownsAccount(db, userId, compte));
...
export async function dismissNotification(id: string): Promise<void> {
  ...
  await pourMoi(async (db, userId) => {
    if ((await siennes(db, userId, [id])).length === 0) return;
...
export async function dismissAllNotifications(ids: string[]): Promise<void> {
  await pourMoi(async (db, userId) => {
    const miennes = await siennes(db, userId, ids);
