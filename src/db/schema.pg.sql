...
-- NULL jusqu'à une synchronisation rapportant le solde comptabilisé.
...
-- AUTOMATION_SCHEMA_START — installation additive : scripts/appliquer-automatisations.mjs
CREATE TABLE IF NOT EXISTS automation_rules (
  ...
  group_id INTEGER NOT NULL REFERENCES groups(id) ON DELETE CASCADE,
  line_id INTEGER REFERENCES group_lines(id) ON DELETE CASCADE,
  enabled BOOLEAN NOT NULL DEFAULT TRUE,
  revision INTEGER NOT NULL DEFAULT 1,
  CHECK (min_amount IS NULL OR max_amount IS NULL OR min_amount <= max_amount)
);
CREATE INDEX IF NOT EXISTS automation_rules_account_idx ON automation_rules(account_id, id);
-- Un événement par transaction : une correction manuelle ne relance pas la règle.
-- Les noms sont conservés pour qu'une règle supprimée ne réécrive pas l'histoire.
...
CREATE INDEX IF NOT EXISTS automation_events_account_idx ON automation_events(account_id, created_at DESC);
GRANT SELECT, INSERT, UPDATE, DELETE ON automation_rules, automation_events TO budget_app;
GRANT USAGE, SELECT ON SEQUENCE automation_rules_id_seq, automation_events_id_seq TO budget_app;
ALTER TABLE automation_rules ENABLE ROW LEVEL SECURITY;
ALTER TABLE automation_events ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS a_soi ON automation_rules;
CREATE POLICY a_soi ON automation_rules
  USING (EXISTS (SELECT 1 FROM accounts a WHERE a.id = automation_rules.account_id AND a.user_id = app_user_id()))
  WITH CHECK (EXISTS (SELECT 1 FROM accounts a JOIN groups g ON g.account_id = a.id
    WHERE a.id = automation_rules.account_id AND a.user_id = app_user_id() AND g.id = automation_rules.group_id
    AND (automation_rules.line_id IS NULL OR EXISTS (SELECT 1 FROM group_lines l WHERE l.id = automation_rules.line_id AND l.group_id = g.id))));
DROP POLICY IF EXISTS a_soi ON automation_events;
CREATE POLICY a_soi ON automation_events
  USING (EXISTS (SELECT 1 FROM accounts a WHERE a.id = automation_events.account_id AND a.user_id = app_user_id()))
  WITH CHECK (EXISTS (SELECT 1 FROM accounts a JOIN transactions t ON t.account_id = a.id
    WHERE a.id = automation_events.account_id AND a.user_id = app_user_id() AND t.id = automation_events.transaction_id));
