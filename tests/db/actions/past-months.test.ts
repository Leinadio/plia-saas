...
// réellement écrit.
const amountsOf = async (groupId: number) =>
  (toDatedBudgets(await listBudgetAmounts(db))[groupId] ?? []).map((e) => [e.effectiveMonth, e.amount]);

describe("setGroupAmount", () => {
  test("écrit dans un mois passé", async () => {
    const gid = await insertGroup(db, "a1", "Courses", "out", 300, "2026-01", null);
    await setBudgetAmount(db, gid, "2026-01", 300);

    await setGroupAmount(gid, "2026-03", 350, "ongoing");

    expect(await amountsOf(gid)).toEqual([["2026-01", 300], ["2026-03", 350]]);
  });
  ...
  test("écrit aussi « ce mois seulement » dans un mois passé", async () => {
    ...
    expect(await amountsOf(gid)).toEqual([["2026-01", 300], ["2026-03", 350]]);
    expect((await listBudgetAmounts(db)).find((b) => b.groupId === gid && b.effectiveMonth === "2026-03")?.scope).toBe("once");
  });
  ...
  test("accepte le mois courant", async () => {
    ...
    expect(await amountsOf(gid)).toEqual([["2026-01", 300], ["2026-07", 350]]);
  });
  ...
  test("accepte un mois futur", async () => {
    ...
    expect(await amountsOf(gid)).toEqual([["2026-01", 300], ["2026-09", 350]]);
  });
  ...
  test("refuse un mois mal formé", async () => {
    ...
    expect(await amountsOf(gid)).toEqual([["2026-01", 300]]);
  });
...
describe("setUncatProvision", () => {
  test("écrit dans un mois passé", async () => {
    await setBudgetAmount(db, 0, "2026-01", 100);

    await setUncatProvision("a1", "2026-03", 150, "ongoing");

    expect(await amountsOf(0)).toEqual([["2026-01", 100], ["2026-03", 150]]);
  });
  ...
  test("accepte le mois courant", async () => {
    ...
    expect(await amountsOf(0)).toEqual([["2026-01", 100], ["2026-07", 150]]);
  });
  ...
  test("refuse un mois mal formé", async () => {
    ...
    expect(await amountsOf(0)).toEqual([["2026-01", 100]]);
  });
});
...
describe("removeGroupAmount", () => {
  ...
  test("retire un changement posé dans un mois passé", async () => {
    const gid = await insertGroup(db, "a1", "Courses", "out", 300, "2026-01", null);
    await setBudgetAmount(db, gid, "2026-01", 300);
    await setBudgetAmount(db, gid, "2026-03", 350);
  ...
  test("accepte de retirer un changement du mois courant", async () => {
    const gid = await insertGroup(db, "a1", "Courses", "out", 300, "2026-01", null);
    await setBudgetAmount(db, gid, "2026-01", 300);
    await setBudgetAmount(db, gid, "2026-07", 350);
...
describe("addGroupLine", () => {
  test("crée une ligne dans un mois passé", async () => {
    ...
    expect(lineAmountInForce(lid, "2026-03", toDatedLineAmounts(await listLineAmounts(db)))).toBe(15);
  });
  ...
  test("accepte de créer une ligne au mois courant", async () => {
    ...
    expect(lid).toBeGreaterThan(0);
    expect(lineAmountInForce(lid, "2026-07", toDatedLineAmounts(await listLineAmounts(db)))).toBe(15);
  });
...
describe("setGroupLineAmount", () => {
  ...
  test("accepte le mois courant", async () => {
    ...
    expect(lineAmountInForce(lid, "2026-07", toDatedLineAmounts(await listLineAmounts(db)))).toBe(12);
  });
...
describe("removeLineAmount", () => {
  test("retire un montant posé dans un mois passé", async () => {
    ...
    expect(lineAmountInForce(lid, "2026-07", toDatedLineAmounts(await listLineAmounts(db)))).toBe(10);
