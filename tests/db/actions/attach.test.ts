...
test("refuse de rattacher une transaction à un récurrent sans ligne", async () => {
  ...
  await insertLine(db, gid, "Sosh Internet", 30.99);
...
test("refuse de rattacher une transaction à une enveloppe qui a des sous-postes", async () => {
  const gid = await insertGroup(db, "a1", "Courses", "out", 400, "2026-01", null);
  await insertLine(db, gid, "Boulangerie", 50);
  const id = await nouvelleTxn();
...
test("accepte le rattachement à une ligne d'un récurrent, groupe parent compris", async () => {
  const gid = await insertGroup(db, "a1", "Abonnements", "out", 0, "2026-01", null);
  const lid = await insertLine(db, gid, "Sosh Internet", 30.99);
  const id = await nouvelleTxn();

  await setGroup(id, gid, lid);

  expect(await rattachement(id)).toEqual({ groupId: gid, lineId: lid });
});
...
test("un refus laisse le rattachement précédent intact", async () => {
  const env = await insertGroup(db, "a1", "Courses", "out", 300, "2026-01", null);
  const rec = await insertGroup(db, "a1", "Abonnements", "out", 0, "2026-01", null);
  await insertLine(db, rec, "Sosh Internet", 30.99);
  const id = await nouvelleTxn();
  await setGroup(id, env, null);

  await setGroup(id, rec, null);

  expect(await rattachement(id)).toEqual({ groupId: env, lineId: null });
});
...
test("refuse une ligne qui n'appartient pas au groupe visé", async () => {
  const a = await insertGroup(db, "a1", "Abonnements", "out", 0, "2026-01", null);
  const b = await insertGroup(db, "a1", "Impôts", "out", 0, "2026-01", null);
  const ligneDeB = await insertLine(db, b, "Impôts", 49);
  const id = await nouvelleTxn();
...
test("refuse de rattacher une transaction à un groupe qui ne vit pas son mois", async () => {
  const juillet = await insertGroup(db, "a1", "Sucreries", "out", 40, "2026-07", "2026-07");
...
test("accepte le rattachement au mois où le groupe vit", async () => {
  const juillet = await insertGroup(db, "a1", "Sucreries", "out", 40, "2026-07", "2026-07");
  const id = await nouvelleTxn(); // 2026-07-23

  await setGroup(id, juillet, null);

  expect(await rattachement(id)).toEqual({ groupId: juillet, lineId: null });
});
