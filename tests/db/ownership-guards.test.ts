// Les gardes qui répondent « est-ce à lui ». Tout le reste de la protection en
// écriture s'y adosse, donc elles doivent être justes sur les cas tordus autant que
// sur les cas simples : l'objet d'un autre, l'objet qui n'existe pas, l'objet dont le
// compte n'a pas de propriétaire.
import { expect, test } from "vitest";
import { createTestDb } from "../helpers/pg";
import { dbFrom, type Db } from "../../src/db/pg";
import { upsertAccount } from "../../src/db/repositories/accounts";
import { insertGroup, insertLine } from "../../src/db/repositories/groups";
import { upsertTransaction } from "../../src/db/repositories/transactions";
import {
  ownsAccount, ownsGroup, ownsLine, ownsTransaction, ownsGroupOrUncategorized,
} from "../../src/db/repositories/ownership";

const MOI = "u-moi";
const AUTRE = "u-autre";

async function base(): Promise<{ db: Db; gMoi: number; gAutre: number; lMoi: number }> {
  const db = dbFrom(await createTestDb());
  await upsertAccount(db, { id: "moi", name: "CIC", iban_masked: null, balance: 0, currency: "EUR", last_synced: null }, MOI);
  await upsertAccount(db, { id: "autre", name: "SG", iban_masked: null, balance: 0, currency: "EUR", last_synced: null }, AUTRE);
  // Un compte hérité, sans propriétaire : il n'est à personne.
  await db.run(`INSERT INTO accounts (id, name, iban_masked, balance, currency, last_synced)
     VALUES ('orphelin', 'Vieux', NULL, 0, 'EUR', NULL)`);

  const gMoi = await insertGroup(db, "moi", "Courses", "out", 400, "2026-01", null);
  const gAutre = await insertGroup(db, "autre", "Loyer", "out", 900, "2026-01", null);
  const lMoi = await insertLine(db, gMoi, "Boulangerie", 50);
  await insertGroup(db, "orphelin", "Perdu", "out", 10, "2026-01", null);

  await upsertTransaction(db, { id: "t-moi", account_id: "moi", date: "2026-08-01", amount: -20, label: "X" });
  await upsertTransaction(db, { id: "t-autre", account_id: "autre", date: "2026-08-01", amount: -20, label: "Y" });
  return { db, gMoi, gAutre, lMoi };
}

test("un compte est à son propriétaire et à personne d'autre", async () => {
  const { db } = await base();
  expect(await ownsAccount(db, MOI, "moi")).toBe(true);
  expect(await ownsAccount(db, MOI, "autre")).toBe(false);
  expect(await ownsAccount(db, MOI, "orphelin")).toBe(false);
});

test("une dépense suit son compte", async () => {
  const { db, gMoi, gAutre } = await base();
  expect(await ownsGroup(db, MOI, gMoi)).toBe(true);
  expect(await ownsGroup(db, MOI, gAutre)).toBe(false);
  expect(await ownsGroup(db, AUTRE, gMoi)).toBe(false);
});

test("un sous-poste suit sa dépense, qui suit son compte", async () => {
  const { db, lMoi } = await base();
  expect(await ownsLine(db, MOI, lMoi)).toBe(true);
  expect(await ownsLine(db, AUTRE, lMoi)).toBe(false);
});

test("une transaction suit son compte", async () => {
  const { db } = await base();
  expect(await ownsTransaction(db, MOI, "t-moi")).toBe(true);
  expect(await ownsTransaction(db, MOI, "t-autre")).toBe(false);
});

// Un objet qui n'existe pas répond comme un objet qui n'est pas à soi. Distinguer les
// deux dirait à l'appelant quels numéros existent, ce qui est déjà un renseignement.
test("ce qui n'existe pas n'appartient à personne", async () => {
  const { db } = await base();
  expect(await ownsGroup(db, MOI, 99999)).toBe(false);
  expect(await ownsLine(db, MOI, 99999)).toBe(false);
  expect(await ownsTransaction(db, MOI, "jamais-vue")).toBe(false);
  expect(await ownsAccount(db, MOI, "jamais-vu")).toBe(false);
});

// Le groupe 0 n'est pas un groupe : il désigne les non catégorisés d'un compte, dont
// la provision se règle comme un budget. C'est donc le compte qui décide.
test("le groupe zéro se juge sur son compte", async () => {
  const { db, gMoi } = await base();
  expect(await ownsGroupOrUncategorized(db, MOI, 0, "moi")).toBe(true);
  expect(await ownsGroupOrUncategorized(db, MOI, 0, "autre")).toBe(false);
  expect(await ownsGroupOrUncategorized(db, MOI, 0, null)).toBe(false);
  expect(await ownsGroupOrUncategorized(db, MOI, gMoi, null)).toBe(true);
});
